<div class="row">
    <div class="col-xs-12 col-md-12">
        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
            ...
            <textarea id="name"  class="form-control required validate-min-length" data-min-length="2" name="description" value="{{ old('description') }}" required autofocus>{{ old('description') }}</textarea>
        ...
        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
            ...
            <input type="text" id="name"  class="form-control required validate-min-length" data-min-length="2" name="price" value="{{ old('price') }}" required autofocus>
            @if ($errors->has('price'))
        ...
        <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
            ...
            <div>
                <label class="radio-inline">
                    <input type="radio" name="type" value="0" @if(old('type')==0)checked="checked"@endif>{{__("app.credit")}}
                </label>
                <label class="radio-inline">
                    <input type="radio" name="type" value="1" @if(old('type')==1)checked="checked"@endif>{{__("app.debit")}}
                </label>
            </div>
            @if ($errors->has('type'))
        ...
        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
            ...
            <input type="date" id="name"  class="form-control required validate-min-length" data-min-length="2" name="date" value="{{ old('date') }}" required autofocus>
            @if ($errors->has('date'))
        ...
        <div class="form-group {{ $errors->has('chart_id') ? 'has-error' : '' }}">
            ...
            <select id="name"  class="form-control required validate-min-length" data-min-length="2" name="chart_id" required autofocus>
                ...
                @foreach($charts as $chart)
                    <option value="{{$chart->id}}"  @if(old("chart_id") == $chart->id)selected="selected"@endif>{{$chart->title}}</option>
                @endforeach
            </select>
